<script lang="ts">
	...
	// `page.data.user` from the root `+layout.server.ts` load.
	// Using `$page` makes it reactive across client-side navigations
	// (otherwise the header can lag behind until a full refresh).
...
<div class="w-full">
	<header class="grid grid-cols-2 sm:grid-cols-[auto_1fr_auto] items-center py-3 px-4 sm:px-8 bg-[#c41e3a] text-white">
		...
		<nav class="hidden justify-end sm:flex sm:items-center flex-wrap gap-y-2 w-full">
			...
			{#if user}
				<a href="/userchat" class="hover:underline font-medium text-base mx-2">Messages</a>
			{/if}
